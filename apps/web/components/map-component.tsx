...
export function MapComponent({
  ...
  const flyToThePosition = useCallback((longitude : number, latitude : number) => {
    if(mapRef.current){
    ...
    <div className="relative h-full w-full bg-gray-100 dark:bg-gray-800">
      ...
      >
       {stations.map(station  => (
         <Marker longitude={station.lng} latitude={station.lat} key={station.id} onClick={() => {
          onSelectStation(station.id)
         }}>
         <MapPin className={`w-8 h-8 ${selectedStation === station.id ? "text-red-600" : "text-gray-800"}`} />
         </Marker>
       ))}
      </Map>
            ...
            {stations
              ...
              .map((station) => (
                ...
                >
                  ...
                  <Button size="sm" onClick={() => flyToThePosition(station.lng, station.lat)}>Navigate</Button>
                </div>
              ))}
          </CardContent>
        </Card>
