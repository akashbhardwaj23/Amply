...
import { getTokenBal } from "@/utils/tokenbalance";
...
export function ChargeButton({
  ...
  const [isconfirmCharged, setIsConfirmCharged] = useState(false);
  ...
  useEffect(() => {
    if (isCharging) {
      ...
      const updateProgress = () => {
        ...
        if (newProgress >= 99) {
          console.log("escrow pda is ", escrowPDA)
          console.log("amount in lamports is ", amountInLamports)
  ...
  const handleConfirmCharged = async () => {
    if (!program || !phantom || !phantom.publicKey) {
      console.error('Wallet not connected');
      return;
    }
